...
import { sendKeysendPayment } from '../../utils/webln';
import { fiatToSats } from '../../utils/webln';

export default function KeysendPayment() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!destination.trim()) {
        throw new Error('Please enter a valid node pubkey');
      }
      ...
      const response = await sendKeysendPayment(destination.trim(), satAmount);
      setSuccess(true);
  ...
  return (
    <div className="card p-4 sm:p-6">
      <h2 className="text-lg sm:text-xl font-bold mb-4">Keysend Payment</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            className="w-full p-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded-md text-xs sm:text-sm font-mono"
            placeholder="03...."
          />
        </div>
        ...
        >
          {isLoading ? 'Processing...' : 'Send Keysend Payment'}
        </button>
      </form>
